...
export default function App() {
  ...
  return (
    <div className="app">
      ...
      {/* Main */}
      <div className="main">
        <header style={{ padding: '1rem 1.5rem', borderBottom: '1px solid #E5E7EB', display: 'flex', alignItems: 'center', gap: '1rem', background: 'white' }}>
          <button onClick={() => setSidebarOpen(true)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '0.25rem' }}><Menu size={24} color="#374151" /></button>
          <div style={{ flex: 1 }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          {error && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              style={{ textAlign: 'center', padding: '0.5rem', color: '#DC2626', fontSize: '0.875rem' }}>
              ⚠️ {error}
            </motion.div>
          )}
        ...
        <div style={{ padding: '1rem 1.5rem', borderTop: '1px solid #E5E7EB', background: 'white' }}>
          {error && <div style={{ fontSize: '0.75rem', color: '#DC2626', marginBottom: '0.5rem' }}>⚠️ {error}</div>}
          ...
          <p style={{ fontSize: '0.75rem', color: '#9CA3AF', textAlign: 'center', marginTop: '0.5rem' }}>按 Enter 发送 · Shift+Enter 换行</p>
        </div>
      </div>
